...
/** @const {string} CMD_SHELL - The shell command to compare with. */
const CMD_SHELL = '';
...
`;
	...
	cmdData.cmd_act = `node ${cmdPath} -uxy`;
	cmdData.cmd_exp = '';
	cmdData.cmd_inp = '';
...
`;
	...
	cmdData.cmd_act = `node ${cmdPath} -tu`;
	cmdData.cmd_exp = '';
	cmdData.cmd_inp = '';
...
async function makeTest(obj){
	...
	if(out_exp !== null)
		out_exp.stderr = out_exp.stderr.replace(new RegExp(CMD_SHELL, 'g'), CMD);
	else{
