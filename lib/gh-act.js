...
/** @var {number} debugSeq - Run in debug mode */
let debugSeq = 0;
...
(() => {
	...
	try{
		debugSeq && console.error(`${String(debugSeq++).padStart(3, 0)}`, '... read authorization token file - Start');
		authToken = fs.readFileSync(authTokenFile, 'utf8');
		debugSeq && console.error(`${String(debugSeq++).padStart(3, 0)}`, '... read authorization token file - Finish');
	}
	catch(e){
		debugSeq && console.error(`${String(debugSeq++).padStart(3, 0)}`, '... read authorization token file - Failure');
	}
...
async function processCmd(test){
	...
	try{
		...
		try{
			githubUserEvents = await requestAPI(`${githubUserObj.events_url.replace(/\{.*\}/g, '')}`, cacheUserEvents.etag);
